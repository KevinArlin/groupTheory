...
struct animation {
  ...
  int merge(int loops=0, real[] delays, string format="gif",
            string options="", bool keep=settings.keep) {
    string args="-loop " +(string) loops+
      " -alpha Off -dispose Background "+options;
    for(int i=0; i < files.length; ++i)
      args += " -delay " +string(delays[i]/10) +" "+files[i];
  ...
  int movie(enclosure enclosure=NoBox, int loops=0,
            real delay=animationdelay, real firstDelay=animationdelay, real lastDelay=animationdelay,
	    real[] delays = new real[],
    ...
    if (delays.length == 0) {
    real[] middle = array(files.length-2,delay);
    delays.push(firstDelay);
    delays.append(middle);
